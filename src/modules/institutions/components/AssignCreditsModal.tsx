...
import Table from "./AssignCreditsTable";
...
import { UPDATE_ORG_BALANCE } from "@/graphql/mutations/updateOrgBalance";
...
const AssignCreditsModal: React.FC = () => {
  ...
  const [updateOrgBalance] = useMutation(UPDATE_ORG_BALANCE, {
    onCompleted: () => {
      closeModal();
    },
    onError: (err) => {
  ...
  const handleProceed = async () => {
    try {
      ...
      await updateOrgBalance({
        variables: { _in: selectedIds, balance: amt },
      });
    } catch (e) {
      console.error(e);
    }
        ...
        <div className="flex flex-col gap-2">
          <div className="flex gap-x-2">
            ...
            <p className="font-normal text-xs text-right">
              {selectedIds.length ? selectedIds.length : "XXXX"}
            </p>
          </div>
          <div className="flex gap-x-2">
